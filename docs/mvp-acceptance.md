...
| 9 | Two different agent tokens create separately attributed audit events, can be revoked independently, and never appear in logs or exports. | **covered** (audit + revoke); **not implemented** (export) | `internal/httpapi/exit_criterion_test.go`; `internal/service/agent_test.go` (`TestAgentLifecycleEmitsActorAuditTrail`, `TestAgentTokenAuditEventNeverCarriesTokenValue`, both new in Phase 6 Step 1); `internal/httpapi/security_test.go` (`TestBearerTokenNeverAppearsInLogOutput`). | "Never appear in exports" has no export mechanism yet — Phase 6 Step 4. |
...
| 16 | Backup and restore preserve records, attachments, references, versions, audit history, and checksums; portable export/import is validated separately. | **not implemented** | No backup/restore/export/import code exists. | Phase 6 Steps 3-4, closed out by Step 11's drill. |
...
- **Covered:** 10 / 17 (2, 4, 6, 7, 8, 11, 12, 13, 14, and 9's audit/revoke half)
- **Implemented, untested as a scenario:** 4 / 17 (1, 3, 5, 10)
- **Not implemented:** 3 full rows (15, 16, 17), plus 9's export half
