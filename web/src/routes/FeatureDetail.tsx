...
import { getFeature, listFeatureBacklinks, listFeatureLinks } from '../api/features'
...
export default function FeatureDetail() {
  ...
  useEffect(() => {
    ...
    setBacklinks(null)
    setError(null)
    Promise.all([getFeature(ref), listFeatureLinks(ref), listFeatureBacklinks(ref)])
      .then(([f, l, b]) => {
        setFeature(f)
        setLinks(l.links)
        ...
        <ul>
          {backlinks.map((b) => (
            <li key={`${b.ref}-${b.comment_id ?? 'body'}`}>
              <Link to={/-F\d+$/.test(b.ref) ? `/features/${b.ref}` : `/tickets/${b.ref}`}>
                {b.ref}
              </Link>
